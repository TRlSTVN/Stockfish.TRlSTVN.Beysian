...
// Undo a normal move and keep the NNUE accumulator stack in sync
void Search::Worker::undo_move(Position& pos, const Move move) {
    pos.undo_move(move);
    accumulatorStack.pop();
}

// Keep NNUE accumulator stack in sync for null moves
void Search::Worker::do_null_move(Position& pos, StateInfo& st) {
    pos.do_null_move(st, tt);
    // Signal side-to-move flip to NNUE accumulator (expected as a single synthetic update)
    DirtyPiece stmFlip{};
    stmFlip.pc = NO_PIECE;  // sentinel used by accumulator to mean "STM flip"
    // (fields like from/to/captured are ignored for NO_PIECE in most branches)
    accumulatorStack.push(stmFlip);
}
