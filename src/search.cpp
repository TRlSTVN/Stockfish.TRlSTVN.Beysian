...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const auto  cntcv =
      m.is_ok() ? (*(ss - 2)->continuationCorrectionHistory)[g_bayesCfg.enabled
                                                                ? CapturedPiecePre(pos, m)
...
moves_loop:  // When in check, search starts here// Step 12. A small ProbCut idea (master behavior). Run only when Bayes is disabled
            ...
            while ((move = mp.next_move()) != Move::none())
            {
                ...
                if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
                {
                    ...
                    {
                        Piece capturedPiece = g_bayesCfg.enabled ? CapturedPiecePre(pos, move)
                ...
                {
                    Value singularBeta =
                      ttData.value - (56 + 81 * (ss->ttPv && !PvNode)) * depth / 60;
        ...
        Value Search::Worker::qsearch(Position & pos, Stack * ss, Value alpha, Value beta) {
            ...
            Square prevSq =
              ((ss - 1)->currentMove).is_ok() ? ((ss - 1)->currentMove).to_sq() : SQ_NONE;
            ...
            while ((move = mp.next_move()) != Move::none())
            {
                    ...
                    && (Bqs.qsMaxGatedCaptures != 0 && qsGated < Bqs.qsMaxGatedCaptures))
                {
                          ...
                          && PieceValue[capPre] < Bqs.qsMinCapturedValueCp))
                    {
                        ...
                        int         histTerm =
                          captureHistory[movedPre][move.to_sq()][type_of(capPre)] / qsDiv;
                        if (Bqs.histCapCp > 0)
                        {
                        ...
                        int muCp = int(ss->staticEval)
                                 + (capPre == NO_PIECE ? 0 : g_capWeightVal[capPre]) + histTerm
                                 + Bqs.muBiasCp + qsMuNodeBiasCp + promoDelta;
                        muCp      = std::clamp(muCp, -30000, 30000);
                        ...
                        {
                            int rhsLocal =
                              (ttData.move && move == ttData.move ? qsThrQ8Best : qsThrQ8);
                            if (move.type_of() == PROMOTION)
        ...
        Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
            ...
            return reductionScale - delta * 757 / rootDelta + !i * reductionScale * 218 / 512
                 + 1200;
        }
        ...
        void update_all_stats(const Position& pos,
            ...
            {
                movedPiece    = pos.moved_piece(move);
                capturedPiece = type_of(g_bayesCfg.enabled ? CapturedPiecePre(pos, move)
                                                           : pos.piece_on(move.to_sq()));
        ...
        void SearchManager::check_time(Search::Worker & worker) {
            ...
            if (
              ...
              && ((worker.limits.use_time_management()
                   && (elapsed > tm.maximum() || stopOnPonderhit))
        ...
        void SearchManager::pv(Search::Worker & worker, const ThreadPool& threads,
            ...
            for (size_t i = 0; i < multiPV; ++i)
            {
                ...
                if (is_decisive(v) && std::abs(v) < VALUE_MATE_IN_MAX_PLY
                    && ((!rootMoves[i].scoreLowerbound && !rootMoves[i].scoreUpperbound)
                        || isExact))
